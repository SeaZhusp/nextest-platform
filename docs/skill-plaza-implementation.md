...
内存 dict: skill_id → BaseSkill      # 唯一执行入口
        ...
        ↓ 新增：DB 表 skills
skill_id + 展示字段 + is_published（上架则出现在技能广场）
        ↓
GET /skill-plaza/...                 # 只读
GET /admin/skills/...                # CRUD，仅 admin
```
...
- `app/models/skill.py`：ORM `Skill`，表名 `skills`，继承 `ModelBase`。
- `app/repositories/skill_repository.py`：CRUD、列表过滤、分页。
- `app/services/skill_service.py`：与 registry 校验、详情阅读量 +1；上架即广场可见。
- `app/api/endpoints/skill_plaza.py`：`prefix="/skill-plaza"`，登录用户可访问列表/详情。
- `app/api/endpoints/admin_skills.py`：`prefix="/admin/skills"`，`Depends` 管理员。
- `app/schemas/skill.py`：Pydantic 入参/出参。
...
- `GET /admin/skills`：分页（含未上架）。
- `POST /admin/skills`
- `PUT /admin/skills/{id}`
- `DELETE /admin/skills/{id}`（软删除，与现有规范一致）
...
- **技能管理**：`/system/skills`，`meta.role: 'admin'`。
...
| `views/system/skills/index.vue` | Table + 表单：`skill_id`、`is_published` 上架等；`skill_id` 从 `GET /skills` 选择。 |

### 5.3 API 封装

- `frontend/src/api/skillPlaza.ts`、`adminSkills.ts`。
