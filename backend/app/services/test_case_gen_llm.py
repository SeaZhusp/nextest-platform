...
def get_system_prompt_for_test_case_gen() -> str:
    """随配置生成 system 提示（含最少用例条数）。保留函数名便于 skills/文档引用。"""
...
def build_messages(user_text: str) -> list[dict[str, Any]]:
    return [
        {"role": "system", "content": get_system_prompt_for_test_case_gen()},
...
def _try_parse_json_array_from_string(s: str) -> list[Any] | None:
    """从字符串中解析**第一个**完整 JSON 数组（避免贪婪正则截断嵌套结构）。"""
    s = s.strip()
    if not s:
        return None
    try:
...
def parse_test_cases_from_llm_text(raw: str) -> list[TestCaseItem]:
    ...
    min_n = int(settings.agent_min_generated_test_cases)
    if len(arr) < min_n:
        raise ValueError(f"用例数量不足 {min_n} 条（当前 {len(arr)}），可在环境变量 AGENT_MIN_GENERATED_TEST_CASES 中下调下限")
...
async def generate_test_cases_from_user_text(
    user_text: str,
    config: LlmInvokeConfig,
    *,
    chat_messages: list[dict[str, Any]] | None = None,
) -> tuple[list[TestCaseItem], str]:
    """非流式：一次请求拿全文再解析；返回 (用例列表, 模型原文)。"""
    messages = chat_messages if chat_messages is not None else build_messages(user_text)
    content = await chat_completion_content(messages, config=config)
    try:
        return parse_test_cases_from_llm_text(content), content
    except Exception as e:
